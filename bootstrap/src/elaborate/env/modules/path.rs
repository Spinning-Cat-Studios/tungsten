...
mod tests {
    ...
    /// ADR 10.5.26m: the `raw != resolved` guard relies on root.join(raw) == raw.
    /// If this invariant breaks, absolute imports would get false-positive warnings.
    #[test]
    fn test_root_join_preserves_absolute_path() {
        let root = ModulePath::root();
        let raw = ModulePath::new(vec!["parser".into(), "exprs".into(), "result".into()]);
        let child = root.join(&raw);
        assert_eq!(
            child, raw,
            "root.join(raw) should equal raw for absolute imports"
        );
    }

    /// ADR 10.5.26m: child != raw when current module is non-root (relative import).
    #[test]
    fn test_nested_join_differs_from_raw() {
        let current = ModulePath::new(vec!["parser".into(), "exprs".into()]);
        let raw = ModulePath::from_name("result");
        let child = current.join(&raw);
        assert_ne!(
            child, raw,
            "nested join should differ from raw (genuinely relative)"
        );
        assert_eq!(
            child,
