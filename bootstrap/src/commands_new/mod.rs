...
mod cache;
pub use cache::{cmd_clean, cmd_cache_clean_all, cmd_cache_stats, cmd_cache_prune};
...
pub fn cmd_check(file: &PathBuf, opts: &CheckOptions) -> ExitCode {
    let CheckOptions { verbose, no_cache, max_errors, dump_types, json } = *opts;
    ...
    let pipeline_opts = PipelineOpts { mode: Mode::Check, verbose, dump_types };
            ...
            if json {
                let report = tungsten_bootstrap::driver::diagnostics::hints::JsonDiagnosticReport {
                    errors: vec![tungsten_bootstrap::driver::diagnostics::hints::JsonError {
                        ...
                        message: "elaboration failed (use non-JSON mode for detailed errors)".to_string(),
                        file: Some(file.display().to_string()),
                        line: None,
                        hints: vec![tungsten_bootstrap::driver::diagnostics::hints::JsonHint {
                            command: format!("tungsten doctor suggest-tools \"elaboration error\""),
                            reason: "Get diagnostic suggestions for elaboration failures".to_string(),
                        }],
                    }],
                };
                println!("{}", serde_json::to_string_pretty(&report).unwrap());
...
pub fn cmd_run(file: &PathBuf, verbose: bool, no_cache: bool, max_errors: usize, dump_types: bool) -> ExitCode {
    let pipeline_opts = PipelineOpts { mode: Mode::Run, verbose, dump_types };
    match driver::run_file_with_options(file, &pipeline_opts, no_cache, max_errors) {
